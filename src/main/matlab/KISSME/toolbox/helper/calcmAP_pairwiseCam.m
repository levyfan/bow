function [ mAP, r1_precision ] = calcmAP_pairwiseCam( M, data_test, data_query, indexMap, testCam, queryCam, nModel )
...
alldist = zeros(nTest,nQuery,nModel);
for i = 1:nModel
    i
    alldist(:,:,i) = sqdist(data_test, data_query, M{i});
end
...
for k = 1:size(dist, 2)
    k
    for i = 1:nTest
        dist(i,k) = alldist(i, k, indexMap(testCam(i),queryCam(k)));
    end
